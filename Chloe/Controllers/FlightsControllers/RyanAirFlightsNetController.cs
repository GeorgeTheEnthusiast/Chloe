...
//            actions.Build().Perform();
        ...
        {
            IWebElement webElement = _driver.FindElement(By.CssSelector("div[translate='common.all_countries']"));
            webElement.Click();
...
//            actions.Build().Perform();
        ...
        private List<City> GetAllCities()
        {
            ...
            var citiesWebElements =
                webElement.FindElements(By.TagName("span"));
        ...
        private void CreateNet(City cityFrom)
        {
            ...
            var countriesWebELement = paneLeftWebElement.FindElements(By.TagName("span"));

            foreach (var countryWebElement in countriesWebELement)
            {
                var parent = countryWebElement.FindElement(By.XPath(".."));
                string className = parent.GetAttribute("class");

                if (className.ToLower().Contains("unavailable"))
                {
                    continue;
                }
                ...
                var citiesWebElement = paneRightWebElement.FindElements(By.TagName("span"));

                foreach (var cityWebElement in citiesWebElement)
                {
                    var parentCity = cityWebElement.FindElement(By.XPath(".."));
                    string cityWebElement_Name = parentCity.Text.Trim();
                    ...
                    //TODO na potrzeby prezentacji zakomentowane_netCommand.Merge(net);
                }
            }
        }
    }
