...
namespace Flights.Controllers.FlightsControllers
{
    public class RyanAirWebSiteController : IWebSiteController
    {
        ...
        private void FillCityTo(string cityName)
        {
            ...
            //IWebElement toCityWebElement = _driver.FindElement(By.CssSelector("input[placeholder='Lotnisko docelowe']"));
        ...
        private void CloseAllAdvertisements()
        {
            try
            {
                var advertisements = _driver.FindElement(By.TagName("promo-banner"))
        ...
        private List<Flight> CollectFlights(SearchCriteria searchCriteria)
        {
            ...
            var slideActive = _webDriverWait.Until(
                x => x.FindElement(By.CssSelector("div[class='scroller']"))
                .FindElements(By.ClassName("slide"))
                .ToList()
                .FirstOrDefault(y => y.GetAttribute("class").Contains("active")));
            var flightSlides = _webDriverWait.Until(x => x.FindElement(By.CssSelector("div[class='scroller']")).FindElements(By.ClassName("slide")));
